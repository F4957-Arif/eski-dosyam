...
// Task ->girilen bir mail hesabının @ sembolune kadar olan karakterlerni print eden code create ediniz.
        ...
        System.out.println("Lütfen mail hesabını giriniz : ");
        String mail = sc.nextLine();
        for (int i = 0; i <mail.length() ; i++) {//0 indexsten mail son indexse kadar her bır karekterı dongüye alır
            char c =mail.charAt(i);//dongüdeki her bir karekterı c ye atar
            if (c=='@'){//c de @ karekterı var ıse if blok run edılır
                break;//if blok lopp u donguyu sonlandırı
            }
            System.out.print(c);//lopptaki elde edılen karekterlerı c ye atayıp prın eder
        }
