package Day04;

import java.util.Scanner;

public class Q02_StringManipulation {
    //Scanner kullanarak iki ayri deger giriniz ve bu iki kelimeyi method kullanarak birlestiriniz.

//yukardaki ornekte verilen ilk ve ikinci degiskenlerinin ilk harflerini atip birlestiriniz.

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("String 1 :");
        String str1 = sc.nextLine().toUpperCase();
        System.out.println("String 2 : ");
        String str2 = sc.nextLine().toUpperCase();
        System.out.println("1. method ile birleşim : "+str1+" "+str2);
        System.out.println("2. method ile birleşim : "+str1.concat(" "+str2));

        String str1Ilksız = str1.substring(1).toUpperCase();
        String str2Ilsız  = str2.substring(1).toUpperCase();

        System.out.println("İlk harfler olmadan birleşimi: " +str1Ilksız+" " +str2Ilsız);











    }

}
